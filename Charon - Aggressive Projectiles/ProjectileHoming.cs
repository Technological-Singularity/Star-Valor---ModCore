...
namespace Charon.StarValor.AggressiveProjectiles {
    partial class ProjectileHoming : MonoBehaviour {
        ...
        public void Initialize(Transform owner, Rigidbody rb, SpaceShip ss, Weapon weapon, Transform target, float speed, float turnRate) {
            ...
            if (trackActive) {
                controlCountAddedError = 20f / Mathf.Max(1, (20 + gunnerLevel));
                if (!ss.CompareTag("Player"))
                    controlCountAddedError /= 4;
        ...
        Quaternion CalculateAimError() {
            ...
            gunnerLevelEffective = gunnerLevel - 3 * gunnerControlTracker.Value;
            ...
            var angle = trackActive ? Quaternion.Angle(rb.rotation, Quaternion.LookRotation(rb.position - targetPredictor.State.pos)) : Mathf.Clamp(35 - gunnerLevelEffective, 15, 60);
